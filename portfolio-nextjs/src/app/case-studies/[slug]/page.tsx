  ...
  return (
    <>
      ...
      <main id="main-content" className="pt-20">
        {/* Hero Section */}
        <section className="bg-gradient-to-br from-[var(--color-primary-900)] via-[var(--color-primary-800)] to-[var(--color-primary-900)] text-white py-20 md:py-28">
          <div className="container mx-auto px-6">
            <ScrollReveal>
              <Link
                href="/case-studies"
                className="inline-flex items-center gap-2 text-[var(--color-accent-400)] hover:text-[var(--color-accent-300)] transition-colors mb-8"
              >
            ...
            <ScrollReveal delay={0.1}>
              ...
              <p className="text-h4 text-[var(--color-neutral-300)] font-normal mb-2">
                {caseStudy.subtitle}
              </p>
              <p className="text-body-lg text-[var(--color-neutral-400)] max-w-3xl">
        ...
        {/* Stats Bar */}
        <section className="bg-[var(--color-surface-secondary)] py-12 border-b border-[var(--color-neutral-200)]">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto text-center">
              {caseStudy.stats.map((stat, index) => (
                <ScrollReveal key={index} delay={index * 0.1}>
                  <div>
                    <span className="text-metric block text-[var(--color-accent-500)]">
                      {stat.value}
                    </span>
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto">
              <ScrollReveal>
                ...
                <p className="text-lead text-[var(--color-neutral-600)] mb-8">
              ...
              <StaggerContainer className="space-y-4">
                {caseStudy.challenge.points.map((point, index) => (
                  <StaggerItem key={index}>
                    <div className="flex items-start gap-4 p-4 bg-[var(--color-surface-secondary)] rounded-lg">
                      <span className="flex-shrink-0 w-8 h-8 rounded-full bg-[var(--color-accent-500)]/10 text-[var(--color-accent-500)] flex items-center justify-center text-body-sm font-semibold">
                        {index + 1}
                      </span>
                      <p className="text-body-md text-[var(--color-neutral-700)]">{point}</p>
                    </div>
                  </StaggerItem>
                ))}
              </StaggerContainer>
                    ...
                    <Card className="h-full">
                      <CardContent className="pt-6">
                        <div className="flex items-center gap-3 mb-4">
                          <div className="w-12 h-12 rounded-lg bg-[var(--color-accent-500)]/10 text-[var(--color-accent-500)] flex items-center justify-center">
                        ...
                        <ul className="space-y-3">
                          {section.points.map((point, pointIndex) => (
                            <li
                              key={pointIndex}
                              className="flex items-start gap-2 text-body-md text-[var(--color-neutral-600)]"
                            >
                              <span className="text-[var(--color-accent-500)] mt-1.5">
                                <svg
                                  width="6"
                                  height="6"
                                  viewBox="0 0 6 6"
                ...
                {caseStudy.results.map((result, index) => (
                  <StaggerItem key={index}>
                    <Card className="text-center h-full">
                      <CardContent className="pt-6">
                        <span className="text-metric block text-[var(--color-accent-500)] mb-2">
                          {result.value}
                        </span>
                        <span className="text-body-sm text-[var(--color-neutral-600)]">
                          {result.label}
        ...
        <section className="py-12 bg-[var(--color-surface-secondary)] border-y border-[var(--color-neutral-200)]">
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto">
              <ScrollReveal className="text-center">
                <h3 className="text-h5 mb-6 text-[var(--color-neutral-500)]">
                  Technologies Used
                </h3>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <StaggerContainer className="grid md:grid-cols-3 gap-6">
                {caseStudy.learnings.map((learning, index) => (
                  <StaggerItem key={index}>
                    <Card className="h-full border-t-4 border-t-[var(--color-accent-500)]">
                      <CardContent className="pt-6">
            ...
            <div className="max-w-4xl mx-auto flex justify-between items-center">
              {prevStudy ? (
                <Link href={`/case-studies/${prevStudy.slug}`} className="group">
                  ...
                  <span className="flex items-center gap-2 text-h5 group-hover:text-[var(--color-accent-500)] transition-colors">
                    <ArrowLeft className="h-4 w-4" />
                    {prevStudy.company}
                  </span>
                </Link>
